    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/70">{total} total projects</p>
        </div>
        {isAdmin && (
            ...
            <div key={project.id} className="glass-card p-5 hover:shadow-lg transition-all cursor-pointer" onClick={() => navigate(`/projects/${project.id}`)}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <h3 className="text-base font-semibold text-gray-800 mt-0.5">{project.title}</h3>
                </div>
              ...
              {isAdmin && (
                <div className="flex justify-end mt-3 pt-3 border-t border-gray-100">
                  <button onClick={(e) => { e.stopPropagation(); setDeleteConfirm(project.id); }} className="p-1.5 hover:bg-red-50 text-red-400 rounded-lg"><Trash2 size={14} /></button>
                </div>
              )}
            </div>
          ))}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.owner_id} onChange={(e) => setForm({ ...form, owner_id: e.target.value })} className="w-full px-3 py-2.5 bg-white/60 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm">
            ...
            <div>
              ...
              <input type="date" value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })} className="w-full px-3 py-2.5 bg-white/60 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
              <input type="date" value={form.due_date} onChange={(e) => setForm({ ...form, due_date: e.target.value })} className="w-full px-3 py-2.5 bg-white/60 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm" />
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
              {staffList.map((u: any) => (
                <button key={u.id} type="button" onClick={() => toggleMember(u.id)}
                  className={`px-3 py-1.5 text-xs rounded-full border transition-all ${form.member_ids.includes(u.id) ? 'bg-primary-500 text-white border-primary-500' : 'bg-white/60 text-gray-600 border-gray-200 hover:border-primary-300'}`}>
                  {u.name}
                </button>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="px-6 py-2.5 bg-gradient-to-r from-primary-500 to-primary-700 text-white text-sm rounded-xl font-medium hover:shadow-lg transition-all">Create Project</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={deleteConfirm !== null} onClose={() => setDeleteConfirm(null)} title="Delete Project" size="sm">
        <p className="text-gray-600 mb-4">Are you sure? This will delete all milestones, tasks, and notes for this project.</p>
